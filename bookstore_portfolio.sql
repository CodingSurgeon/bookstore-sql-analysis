...
-- Q10. Rank customers by total spending using a CTE and window function.
--     The finance team wants a clean leaderboard of customer value
--     without losing lower-ranked customers from the result.
...
-- Q11. Assign each book a price tier using CASE WHEN.
--     The catalog team wants to segment titles into Budget,
--     Mid-Range, and Premium categories for a pricing review.
...
-- Q12. Calculate a running total of revenue ordered by date.
--     This helps the business see how revenue has accumulated
--     over time rather than just the end figure.
...
--     The CRM team wants to identify inactive accounts
...
-- Q14. Find books that have been ordered at least once.
--     Useful for separating active inventory from titles
--     that have had no customer interest.
...
-- Q15. Build a combined activity report showing both
--     customers with no orders and books with no sales
--     in a single result set using UNION ALL.
 
SELECT 'No Orders' AS Flag,
       c.Name      AS Name,
       'Customer'  AS Type
FROM Customers c
WHERE NOT EXISTS (
...
SELECT 'No Sales'  AS Flag,
       b.Title     AS Name,
       'Book'      AS Type
FROM Books b
WHERE NOT EXISTS (
    SELECT 1 FROM Orders o WHERE o.Book_ID = b.Book_ID
)
ORDER BY Type, Name;
 
 
-- Q16. Using a CTE, identify the top-selling book per genre.
--     The buying team wants to know which title leads each
--     category so they can plan stock replenishment by genre.
 
WITH Genre_Sales AS (
    SELECT b.Genre,
           b.Title,
           SUM(o.Quantity) AS Units_Sold,
           RANK() OVER (PARTITION BY b.Genre ORDER BY SUM(o.Quantity) DESC) AS Rank
    FROM Orders o
    JOIN Books b ON o.Book_ID = b.Book_ID
    GROUP BY b.Genre, b.Title
)
SELECT Genre, Title, Units_Sold
FROM Genre_Sales
WHERE Rank = 1
ORDER BY Genre;